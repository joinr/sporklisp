...
'persistent data structures that make clojure cool, so you're stuck with the humble list.
'It DOES have lazy/infinite sequences, so you can play some cool tricks.  The sequence library
'alone is probably worth using overe stock VBA.  Keywords exist, as per Clojure.  I don't have them
...
'sporklisp is actually fast enough for lots of scripting...although I haven't really tested the upper
'bounds of it's reasonable performance (i.e. doing numeric work).  I would not recommend building a monster
...
'or represent lists as chunks of arrays, which provide much better cache coherence and speed.  Even in VBA, it'd be
...
'
'If I "really" wanted a reactive spreadsheet, I'd just wrap one in Swing with clojure
...
'(as long as MS continues to support VBA).  Just like clojure let's me live in Java without
